 ...
 */
public class ElevatedSectorGeometryList extends SectorGeometryList
{
    ...
    public ElevatedSectorGeometryList( Globe globe )
    {
        this.globe = globe;
    }
    ...
    /**
     * Constructs a sector geometry list that contains a specified list of sector
     * geometries.
     * 
     * @param list
     *          the secter geometries to place in the list.
     */
    public ElevatedSectorGeometryList( ElevatedSectorGeometryList list )
    {
        super( list );
        this.globe = list.globe;
    }
     ...
     * 
     * @param sector
     *          the sector spanned by this list.
     */
    @Override
    ...
    /**
     * Indicates that this list's sectors are about to be rendered. When rendering
     * is complete, the
     * {@link #endRendering(gov.nasa.worldwind.render.DrawContext)} must be
     * called.
     * 
     * @param dc
    ...
    public void beginRendering( DrawContext dc )
    {
        ...
        // TODO: add the beginRendering interface to Tessellator in order to
        // eliminate this type test
        if ( getActiveGlobe( dc ).getTessellator( ) instanceof RectangularTessellator ) ( ( RectangularTessellator ) getActiveGlobe( dc ).getTessellator( ) ).beginRendering( dc );
    }
    ...
    public void endRendering( DrawContext dc )
    {
        ...
        if ( getActiveGlobe( dc ).getTessellator( ) instanceof RectangularTessellator ) ( ( RectangularTessellator ) getActiveGlobe( dc ).getTessellator( ) ).endRendering( dc );
    }

    /**
     * Detects the locations of the sector geometries in this list that intersect
     * a specified screen point.
     * <p/>
    ...
    public void pick( DrawContext dc, java.awt.Point pickPoint )
    {
        ...
        try
        {
            ...
            if ( pickedSector == null || pickedSector.getObject( ) == null ) return; // no sector picked
        ...
        finally
        {
            ...
            gl.glShadeModel( GL.GL_SMOOTH ); // restore to default explicitly to avoid
                                             // more expensive pushAttrib
            gl.glDepthFunc( GL.GL_LESS ); // restore to default explicitly to avoid more
                                          // expensive pushAttrib
    ...
    /**
     * Detects the locations of the sector geometries in this list that intersect
     * any of the points in a specified list of screen points.
     * <p/>
     * Note: Prior to calling this method,
     * {@link #beginRendering(gov.nasa.worldwind.render.DrawContext)} must be
     * called.
    ...
    public List<PickedObject> pick( DrawContext dc, List<Point> pickPoints )
    {
        ...
        if ( pickPoints == null || pickPoints.size( ) < 1 ) return null;
        ...
        try
        {
            ...
            if ( this.pickSectors.size( ) < 1 ) return null;

            // Now have each sector determine the pick position for each intersecting
            // pick point.
            gl.glDepthFunc( GL.GL_LEQUAL );
            ...
            for ( Map.Entry<SectorGeometry, ArrayList<Point>> sector : this.pickSectors.entrySet( ) )
            {
                ...
                {
                    if ( po != null ) pickedObjects.add( po );
                }
            }
    ...
    /**
     * Computes a Cartesian point at a specified latitude, longitude and altitude
     * above the terrain.
     * 
     * @param position
     *          the position to compute the Cartesian point for. The altitude
     *          element of the position is considered to be distance above the
     *          terrain at the position's latitude and longitude.
     * 
     * @return the Cartesian point, in meters, relative to an origin of (0, 0, 0).
     *         Will be null if there is no sector geometry in this list for the
     ...
     * 
     * @param latLon
     *          the location of the point to compute.
     * 
     * @return the Cartesian point, in meters, relative to an origin of (0, 0, 0).
     *         Will be null if there is no sector geometry in this list for the
     *         specifed latitude and longitude.
     ...
     * 
     * @param latitude
     *          the latitude of the point to compute.
     * @param longitude
     *          the longitude of the point to compute.
     * 
     * @return the Cartesian point, in meters, relative to an origin of (0, 0, 0).
     *         Will be null if there is no sector geometry in this list for the
     *         specifed latitude and longitude.
     ...
     *          the longitude of the point to compute.
     * @param metersOffset
     *          the distance above the terrain of the point to compute.
     * 
     * @return the Cartesian point, in meters, relative to an origin of (0, 0, 0).
     *         Will be null if there is no sector geometry in this list for the
     *         specifed latitude and longitude.
     ...
     * 
     * @param line
     *          the <code>Line</code> for which an intersection is to be found.
     * 
     * @return the <Vec4> point closest to the ray origin where an intersection
     *         has been found or null if no intersection was found.
     */
    @Override
    public Intersection[] intersect( Line line )
    {
        ...
        {
            if ( sg.getExtent( ).intersects( line ) ) if ( ( hits = sg.intersect( line ) ) != null ) list.addAll( Arrays.asList( hits ) );
        }
     ...
     * individual segments - two <code>Intersection</code> for each, corresponding
     * to each geometry triangle that intersects the given elevation.
     * <p/>
     * Note that the provided bounding <code>Sector</code> only serves as a 'hint'
     * to avoid processing unnecessary geometry tiles. The returned intersection
     * list may contain segments outside that sector.
     * 
     * @param elevation
     *          the elevation for which intersections are to be found.
     * @param sector
     *          the sector inside which intersections are to be found.
     * 
    ...
    public Intersection[] intersect( double elevation, Sector sector )
    {
        ...
        for ( SectorGeometry sg : sglist )
        {
            if ( sector.intersects( sg.getSector( ) ) ) if ( ( hits = sg.intersect( elevation ) ) != null ) list.addAll( Arrays.asList( hits ) );
        }
